define(['angular'],function (angular) {
	...
	profileControllers.controller('AddMemberCtrl', [ '$scope', 'winWebHttpService','$upload','$rootScope','$filter','$cookies','$routeParams',
		...
		$scope.addMember=function (){
			...
			var user=$scope.user;				
			user.createUsername=$cookies.name;
			console.log(user);
			winWebHttpService.querywithParams('index/addUser',user).then(function(newData) {
				$rootScope.userCG=user;
				
				$rootScope.userId=newData.errMsg;
...
//		$scope.fnGetUser = function(username){
		function fnGetUser(username){
			...
			$scope.smokes = [
				               {'id':1,'name':'无（不吸烟）'},
				               {'id':2,'name':'偶尔（平均每日少于3支）'},
				               {'id':3,'name':'一般（平均每日少于10支）'},
				               {'id':4,'name':'嗜好（平均每日10支以上）'},
				               {'id':5,'name':'最近一个月刚戒烟'}
			             ];
			...
			$scope.drinks = [
				               {'id':1,'name':'无（不喝酒）'},
				               {'id':2,'name':'偶尔（平均每日少于1两白酒或1瓶啤酒）'},
				               {'id':3,'name':'一般（平均每日少于3两白酒或3瓶啤酒）'},
				               {'id':4,'name':'嗜好（平均每日3两白酒或3瓶啤酒以上）'},
				            ];
			
			// sport 
			$scope.sports = [
				               {'id':1,'name':'从不运动或者每天运动时间不足10分钟，或运动时心率低于110次/分钟'},
				               {'id':2,'name':'运动偏少，平均每日运动时间不足30分钟'},
				               {'id':3,'name':'经常运动，平均每天运动时间超过30分钟且运动时心率大于110次/分钟'},
				               {'id':4,'name':'运动过量，连续大运动量或运动量增加过快导致全身乏力、运动性头痛等'},
				          ];
			
			// sleep 
			$scope.sleeps = [
				               {'id':1,'name':'睡眠时间保证或睡觉醒来精力充沛'},
				               {'id':2,'name':'入睡困难，入睡时间超过30分钟'},
				               {'id':3,'name':'每日睡眠时间不足6小时'},
				               {'id':4,'name':'睡眠时间充足，但醒来后身体疲劳感还是没有得到改善'},
				          ];
			// address
			...
			$scope.submitHealth=function(){
				...
				var healthJson = {
						tall:$scope.profile.tall,
						weight:$scope.profile.weight,
						isPregnant:$scope.profile.isPregnant ? 1 : 0,
						pregnatWeeks:$scope.profile.pregnatWeeks,
						operationHis:$scope.profile.operationHis,
						disIds:disIds,
						jobId:jobid,
