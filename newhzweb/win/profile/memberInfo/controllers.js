...
define(['angular'],function (angular) {
	...
	var profileControllers = angular.module('winWeb.controllers.profileMemberInfo', ['angularFileUpload','ui.bootstrap','ngCookies']);

	profileControllers.controller('ProfileMemberInfoCtrl', [ '$scope', 'winWebHttpService','$upload','$rootScope','$filter','$cookies','$routeParams',
...
//		$scope.fnGetUser = function(username){
		function fnGetUser(username){
			...
			winWebHttpService.querywithParams('index/getProfileByUsername',{username:username}).then(function(newData) {
				...
				$('[href="#~/profile#tc_profile_3"]').click();
				
				if($routeParams.jump==1){
					$('[href="#~/profile#tc_profile_1"]').click();	
				}else if($routeParams.jump==2){
					$('[href="#~/profile#tc_profile_3"]').click();
				}
				
				
				if(!$scope.call){
					$scope.call=newData.realnameZh;
				}
