...
require.config({
    ...
    shim : {
        'jquery': ['css!../css/black/easyui.css','css!../css/icon.css','css!../css/main.css','../../module/system/js/resourceManger.js'],        
    }		
 ...
 */
require(['jquery'],function($){ //require函数的第一个参数是数组，数组中存放的是模块名（字符串类型），数组中的模块与回调函数的参数一一对应 
	/**
	 ...
	 */
	$("#load").click(function(){
		require(["student1","class1"], function(student,clz) {// student,clz 把这些”全局变量”作为参数传递到模块的实现体里，
			var students = [{name:'杰克',gender:'男'},{name:'rose',gender:'female'}];
			var i=0, ilth = students.length;//for循环的优化写法
			for(;i<ilth;i++){
				console.log(students[i]);
				console.log(student.createStudent(students[i]));
